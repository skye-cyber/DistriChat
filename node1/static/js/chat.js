    ...
    createMessageElement(data) {
        ...
        const messageHtml = `
            <div id="${messageId}" class="message-item ${isCurrentUser ? "message-sent" : "message-received"} animate-fade-in">
                <div class="flex space-x-3 max-w-3xl ${isCurrentUser ? "ml-auto" : ""}">
                    ${!isCurrentUser
                ? `
                    <div class="flex-shrink-0">
                        <div class="w-10 h-10 bg-gradient-to-r from-${data.color || "blue"}-500 to-${data.color || "blue"}-600 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-lg">
                ...
                : ""
                    ...
                    <div class="flex-1 ${isCurrentUser ? "text-right" : ""}">
                        <div class="${isCurrentUser ? "bg-gradient-to-r from-blue-500 to-purple-600 text-white" : "bg-white border border-gray-200"} rounded-2xl px-4 py-3 shadow-sm hover:shadow-md transition-all duration-200">
                            ${!isCurrentUser
                ? `
                            <p class="text-sm font-semibold text-gray-900 mb-1">${data.sender}</p>
                            `
                : ""
            }
                            <p class="${isCurrentUser ? "text-white" : "text-gray-800"} leading-relaxed">${this.escapeHtml(data.message)}</p>
                            <p class="text-xs ${isCurrentUser ? "text-blue-100" : "text-gray-500"} mt-2">
                                ${new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                            </p>
                        </div>

                        <div class="flex items-center space-x-2 mt-2 ${isCurrentUser ? "justify-end" : ""}">
                            ...
                            <button class="text-gray-400 hover:text-gray-600 transition-colors text-sm" onclick="chat.showMessageActions('${messageId}')">
                                ⋮
                            </button>
                        </div>
                    </div>
                ...
                ? `
                    <div class="flex-shrink-0">
                        <div class="w-10 h-10 bg-gradient-to-r from-green-500 to-teal-600 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-lg">
